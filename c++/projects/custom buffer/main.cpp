...
void preview(Arducam::Camera& camera, ArducamImageFrame image) {
    ...
    // push buffer
    bool ret = camera.pushImageBuffer(image.data, buffer_size);
    if (!ret) {
        printf("Failed to push buffer. ret = %x\n", camera.lastError());
        exit_flag = true;
    }
}
...
int main(int argc, char** argv) {
    ...
    buffer_size = camera.width() * camera.height() * (camera.bitWidth() > 8 ? 2 : 1);
    for (int i = 0; i < 5; i++) {
        // alloc memory
        data_buffers.push_back(new uint8_t[buffer_size]);
        // push buffer
        bool ret = camera.pushImageBuffer(data_buffers[i], buffer_size);
        if (!ret) {
            printf("Failed to push buffer. ret = %x\n", camera.lastError());
            return -1;
        }
    ...
    // camera.setMessageCallback(std::bind(error_process, std::ref(camera), std::placeholders::_1,
    // std::placeholders::_2));
    camera.start();
